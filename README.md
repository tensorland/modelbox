...
Currently, they are part of the ModelStore gRPC service, but the plan is to separate it out so that ModelBox can be run in a blob server mode and artifact streaming can be isolated in a separate cluster for performance.
