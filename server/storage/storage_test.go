...
func (s *StorageInterfaceTestSuite) TestCreateModel() {
	...
	blob1 := NewBlobInfo("blender", "/foo/bar", "checksum123", File, 0, 0)
	blob2 := NewBlobInfo("blender", "/foo/pipe", "checksum345", ModelBlob, 0, 0)
	blobs := []*BlobInfo{blob1, blob2}
	m := NewModel("blender", OWNER, NAMESPACE, TASK, description, meta, blobs)
